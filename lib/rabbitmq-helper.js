...
var EnsureExchangeBinding = function ensureExchangeBinding(callback) {
    var _this = this;
    var options = _this.options;
    _this.exchange = _this.connection.exchange(options.exchange.name, options.exchange.options, function (exchange_error) {
        if (exchange_error) {
...
var EnsureQueueBinding = function ensureQueueBinding(processor, callback) {
    var _this = this;
    var options = _this.options;
    _this.queue = _this.connection.queue(options.queue.name, options.queue.options, function (queue_error) {
        if (queue_error) {
...
var EnsureExchange = function ensureExchange(callback) {
    ...
    if (_this.connection) {
        ...
        _this.connection.on('reconnected', function () {
            _this.ensureExchangeBinding(function () {
            });
        });
        _this.ensureExchangeBinding(callback);
    }
};
...
var EnsureRabbitMQ = function ensureRabbitMQ(processor, callback) {
    ...
    if (_this.initialized) {
        ...
        _this.ensureExchange(function (err) {
            if (err) {
                callback({
                    err: err,
                ...
                _this.connection.on('reconnected', function () {
                    _this.ensureQueueBinding(processor, callback);
                });
                _this.ensureQueueBinding(processor, callback);
            }
        });
